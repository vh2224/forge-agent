...
- **Context boundary (after terminal poll, before success/failure):** run `CONTEXT_BOUNDARY=$(node "$FORGE_SCRIPTS_DIR/forge-context-boundary.js" --result "$RESULT_FILE" --cwd "$WORKING_DIR" --plan "$PLAN_PATH")`. Display `.indicator`; carry non-empty `.additional_context` into the next Agent boundary. `.checkpoint_required:true` means the helper atomically created/updated `continue.md` beside the plan and recorded one consumption marker; continue the run at the next safe boundary, never auto-pause. Unknown health yields `ctx ?`, no context, and no checkpoint.
