...
function projectionReceipt(cwd) {
  try {
    ...
    return value && value.version === RENDERER_VERSION && value.stores && typeof value.stores === 'object'
...
function isStale(cwd) {
  ...
  for (const [store, file] of Object.entries(PROJECTION_TARGETS)) {
    try {
      ...
      result[store] = !previous || previous.input !== fragmentSignature(cwd, store).hash ||
...
function writeAll(cwd, opts) {
  ...
  for (const { file, store, render } of targets) {
    ...
    if (fs.existsSync(fpath)) {
      ...
      if (existing === content) {
        skipped.push(file);
        receipt.stores[store] = { input: after.hash, output: digest(content), count: after.count };
        continue;
      }
    }
