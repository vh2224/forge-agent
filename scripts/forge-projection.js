...
const PROJECTION_STATE = '.gsd/forge/projection-state.json';
const RENDERER_VERSION = crypto.createHash('sha256').update(fs.readFileSync(__filename)).digest('hex');
const PROJECTION_TARGETS = { ledger: LEDGER_FILE, decisions: DECISIONS_FILE, memory: MEMORY_FILE };

function digest(content) { return crypto.createHash('sha256').update(content).digest('hex'); }
function projectionReceipt(cwd) {
  try {
    const value = JSON.parse(fs.readFileSync(path.join(cwd, PROJECTION_STATE), 'utf8'));
    // Renderer changes invalidate freshness, not proof of generated bytes.
    return value && typeof value.version === 'string' && value.stores && typeof value.stores === 'object' && !Array.isArray(value.stores)
      ? value : { version: RENDERER_VERSION, stores: {} };
  } catch { return { version: RENDERER_VERSION, stores: {} }; }
}

// Sorted membership + content covers deletion, rename and Git checkout even
// when timestamps/size are preserved. Inspection errors must propagate.
function fragmentSignature(cwd, store) {
  const root = path.join(cwd, '.gsd', store);
  const entries = [];
  function walk(dir) {
    let names;
    try { names = fs.readdirSync(dir, { withFileTypes: true }); }
    catch (error) { if (error.code === 'ENOENT' && dir === root) return; throw error; }
    for (const entry of names.sort((a, b) => a.name.localeCompare(b.name))) {
      const file = path.join(dir, entry.name);
      if (entry.isSymbolicLink()) throw new Error('Projection source symlink cannot prove freshness');
      if (entry.isDirectory()) walk(file);
      else if (entry.name.endsWith('.md')) {
        const stat = fs.statSync(file);
        entries.push([path.relative(root, file), stat.size, stat.mtimeMs, digest(fs.readFileSync(file))]);
      }
    }
  }
  walk(root);
  return { hash: digest(JSON.stringify(entries)), count: entries.length };
}

function isStale(cwd) {
  const schema = guardReadHere(cwd);
  const receipt = projectionReceipt(cwd);
  const result = {};
  for (const [store, file] of Object.entries(PROJECTION_TARGETS)) {
    try {
      const previous = receipt.stores[store];
      result[store] = !previous || (previous.version || receipt.version) !== RENDERER_VERSION || previous.input !== fragmentSignature(cwd, store).hash ||
        previous.output !== digest(fs.readFileSync(path.join(cwd, file)));
    } catch { result[store] = true; }
  }
  if (schema.partial) {
    result.schema_partial = true;
    result.schema_warning = schema.warning;
...
// Receipts prove that an empty store previously generated this exact output.
// An untracked legacy monolith remains protected by the migration guard.
function writeAll(cwd, opts) {
  ...
  const receipt = projectionReceipt(cwd);
  // Preserve the renderer of blocked stores when upgrading an older receipt.
  for (const previous of Object.values(receipt.stores)) {
    if (previous && typeof previous === 'object' && !previous.version) previous.version = receipt.version;
  }
  receipt.version = RENDERER_VERSION;
  ...
  for (const { file, store, render } of targets) {
    ...
    const prior = receipt.stores[store];
    let generated = false;
    try { generated = !!prior && prior.output === digest(fs.readFileSync(path.join(cwd, file))); } catch {}
    if (!force && st && st.state === 'unmigrated' && !generated) {
      blocked.push({
        file,
    ...
    const before = fragmentSignature(cwd, store);
    const rendered = render();
    const after = fragmentSignature(cwd, store);
    if (before.hash !== after.hash) throw new Error(`Projection sources changed during render: ${store}`);
    ...
    if (fs.existsSync(fpath)) {
      ...
      if (existing === content) {
        skipped.push(file);
        receipt.stores[store] = { version: RENDERER_VERSION, input: after.hash, output: digest(content), count: after.count };
        continue;
      }
    }

    writeAtomic(fpath, content, { cwd });
    receipt.stores[store] = { version: RENDERER_VERSION, input: after.hash, output: digest(content), count: after.count };
    written.push(file);
  }

  writeAtomic(path.join(cwd, PROJECTION_STATE), JSON.stringify(receipt), { cwd });
