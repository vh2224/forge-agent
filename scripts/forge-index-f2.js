...
function measureF2(cwd, opts) {
  ...
  return {
    ...
    facts_missed_partial: partial.map((fact) => ({ mem_id: fact.mem_id, storage_key: fact.storage_key, missing_mentions: fact.missing_mentions })),
    ...
    facts_no_mention: facts.filter((fact) => fact.bucket === 'no-mention').map((fact) => ({ mem_id: fact.mem_id, storage_key: fact.storage_key })),
