...
const { claimPathMatches, canonicalizeClaimPath } = require('./forge-parallelism.js');
...
function normalizeScope(raw) {
  ...
  return raw.map((item) => {
    ...
    try { return canonicalizeClaimPath(item); } catch { throw new Error('claim-path-escape'); }
  });
}
