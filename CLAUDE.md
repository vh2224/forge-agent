...
### Maintenance fragment-store@2.0.0 — rollout opt-in per repo, bump 2.0.0 (fecho da milestone)
Fecha a milestone `M-20260706152250-maintenance-fragment`: um segundo caminho de consolidação, além do read/write split original, funde fragmentos soltos finalizados (`.gsd/ledger/`, `.gsd/decisions/`) em buckets rollup imutáveis (`.gsd/archive/{milestones,tasks}-rollup.md`, `.gsd/decisions/_rollup-<trimestre>.md`) via merge determinístico byte-a-byte (`scripts/forge-maintenance.js`, sem LLM). Três gatilhos por contagem (`milestones@30`, `tasks@30`, `ledgerDecisions@100` de trimestre fechado, `detectTriggers` em `scripts/forge-maintenance-baseline.js`) disparam a detecção; a aplicação nunca é automática — `/forge-sweep` e `forge-next` conduzem o handshake completo (aviso vermelho + double-confirm por eixo, `shared/forge-maintenance.md`), e `forge-auto` só anuncia e **para o loop** (exceção sancionada à AUTONOMY RULE) quando um eixo dispara, nunca consolidando inline. **Compat de leitura SAGRADA:** `VALID_SCHEMAS` em `forge-doctor.js` aceita `{fragment-store@1.0.0, fragment-store@2.0.0}` — uma cópia de trabalho `1.0.0` e uma `2.0.0` com buckets projetam `LEDGER.md`/`DECISIONS.md` byte-idênticos; `1.0.0` nunca precisa "terminar de migrar". **Bump 2.0.0 é opt-in e upgrade-only:** `.gsd/SCHEMA-VERSION` permanece `1.0.0` até a primeira aplicação de maintenance que realmente consolide algo (`writtenTargets.length > 0`) — `stampBucketSchema` nunca sobrescreve um carimbo `2.0.0` já existente, e um dry-run ou apply vazio nunca toca o arquivo. **Opt-in por repo:** `maintenance.enabled` (default `false`, `forge-agent-prefs.md § Maintenance Settings`) mantém a feature inteiramente inerte até o repositório optar — `detectMode` retorna `mode: "normal"` sem custo extra de I/O quando desabilitado. **Guard de concorrência VCS** (`scripts/forge-maintenance-vcs.js`) faz precheck read-only antes do apply e resolve colisões por identidade de byte (descarta se idêntico, aborta alto se genuinamente divergente) no commit. **Duas limitações conhecidas, sob triagem, não resolvidas nesta milestone:** (1) a exclusão de unidades ativas em `isFinalized` confia em `forge-runs.active` sem TTL — uma sessão crashada com `active:true` obsoleto pode excluir indefinidamente uma unidade finalizada dos gatilhos/baseline; (2) `baselineAvailable = anyLoose && !anyBucket` não tem limiar — a primeira unidade finalizada de um projeto novo já força `mode: "maintenance"` e um STOP do `forge-auto` no próximo boundary, mesmo sem nenhum eixo ter cruzado o gatilho; se isso deveria virar uma nota informacional não-bloqueante é uma decisão de produto em aberto.
