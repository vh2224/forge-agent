...
8. If you make an architectural decision → append to per-milestone `{WORKING_DIR}/.gsd/milestones/{M###}/{M###}-DECISIONS.md` (M004+). <!-- pre-S05: kept — this is a write path to per-milestone fragment, not a direct read of the global DECISIONS.md monolith. The global `.gsd/DECISIONS.md` is rebuilt from fragments during complete-milestone. --> The global `.gsd/DECISIONS.md` is merged on `complete-milestone` under lockfile. Procedure: `mkdir -p` the dir first; if per-milestone file missing, `Write` it once with header + your row(s); if existing, use **`Edit`** with last-row anchor (never `Write` again — destroys rows). Bash alternative: `cat >> path << 'EOF'`. Legacy fallback if `{M###}` not provided: write to `.gsd/DECISIONS.md` direct.
